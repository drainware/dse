// HeapLocker32.cpp: define las funciones exportadas de la aplicación DLL.
//
...
//	if (sHeapLockerSettings.dwForceTermination)
//	{
//		MessageBox(NULL, (LPCWSTR)lpvArgument, MESSAGEBOX_TITLE, MB_OK | MB_ICONSTOP);
//		TerminateProcess(GetCurrentProcess(), 0);
//	}
//	else if (IDYES == MessageBox(NULL, (LPCWSTR)lpvArgument, MESSAGEBOX_TITLE, MB_YESNO | MB_ICONEXCLAMATION))
//		TerminateProcess(GetCurrentProcess(), 0);
//	else
//		bRet = FALSE;
//	delete pMsg;
...
void ProtectAddresses(HKEY hKeyApplication)
{
	...
	if (NULL != hKeyApplication && ( IS_SUCCESS(RegOpenKeyEx(hKeyApplication, REGISTRY_ADDRESSES, 0L, KEY_READ , &hKey))
		|| IS_SUCCESS(RegOpenKeyEx(hKeyApplication, REGISTRY_ADDRESSES, 0L, KEY_READ | KEY_WOW64_64KEY, &hKey)) )
		)
	{
		dwIndex = 0;
...
void ReadHeapLockerSettingsFromRegistry(void)
{
	...
	if( IS_SUCCESS(RegOpenKeyEx(HKEY_LOCAL_MACHINE, REGISTRY_PATH, 0L, KEY_READ, &hKey)) 
		|| IS_SUCCESS(RegOpenKeyEx(HKEY_LOCAL_MACHINE, REGISTRY_PATH, 0L, KEY_READ | KEY_WOW64_64KEY, &hKey))
		)
	{
		dwValueSize = sizeof(dwValue);
...
HKEY GetApplicationRegKey(void)
{
	...
	__try
	{
		...
		if( IS_SUCCESS(RegOpenKeyEx(HKEY_LOCAL_MACHINE, REGISTRY_PATH_APPLICATIONS, 0L, KEY_READ, &hKey))
			|| IS_SUCCESS(RegOpenKeyEx(HKEY_LOCAL_MACHINE, REGISTRY_PATH_APPLICATIONS, 0L, KEY_READ | KEY_WOW64_64KEY, &hKey))
			)
		{
			dwIndex = 0;
			...
			while (ERROR_NO_MORE_ITEMS != RegEnumKeyEx(hKey, dwIndex++, szKeyName, &dwKeyNameSize, NULL, NULL, NULL, NULL))
			{
				if (!_tcsicmp(pszEXE, szKeyName))
				{
					if( IS_SUCCESS(RegOpenKeyEx(hKey, szKeyName, 0L, KEY_READ , &hKeyApplication))
						|| IS_SUCCESS(RegOpenKeyEx(hKey, szKeyName, 0L, KEY_READ | KEY_WOW64_64KEY, &hKeyApplication))
						)
					{
						...
						if (iIter == iPages && iIter < MAX_PAGES)
						{
							...
							if (!bFirstRun)
							{
								...
								if (NULL != pbFound)
								{
									...
									g_strJSON.Format( "{ \"processname\":\"%ws\",\"startaddress\":\"0x%p\" , \"string\": \"%s\", \"details\":\"string detected\" }", NULL2EmptyString(GetExecutableName()), pbFound, NULL2EmptyString(HexDump(pbFound, 50)) );
									if (FALSE == ThreadedMessageBox(szOutput,g_strJSON))
										if (!sHeapLockerSettings.dwResumeMonitoring)
											return FALSE;
								}
